...
class InferencerProviderFactory:
    ...
    """
    @staticmethod
    def create_inferencer_provider(service: str, model_id: str, region: str = "us-east-1", n_shot_prompts: int = 0, temperature: float = 0.7, n_shot_prompt_guide_obj: Dict[str, List[Dict[str, str]]] = None) -> BaseInferencer:
        if service == "bedrock":
            return BedrockInferencer(model_id, region, n_shot_prompts, temperature, n_shot_prompt_guide_obj)
        elif service == "sagemaker":
            return SageMakerInferencer(model_id, region, n_shot_prompts, temperature, n_shot_prompt_guide_obj)
        else:
